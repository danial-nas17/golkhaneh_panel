...
const CampaignIndex = () => {
  ...
  const fetchDiscounts = async () => {
    setLoading(true);
    try {
      const response = await api.get("/panel/campaigns", {
        params: {
      ...
      message.error("خطا در دریافت  کمپین‌ها");
    }
    setLoading(false);
  };
  ...
  const deleteDiscount = async (id) => {
    try {
      await axios.delete(`/panel/campaigns/${id}`);
      message.success("کمپین با موفقیت حذف شد");
      fetchDiscounts();
    } catch (error) {
      message.error("خطا در حذف کمپین");
    }
  };
    ...
    // },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          ></Button>
          <Popconfirm
            title="آیا از حذف کمپین مطمئن هستید؟"
  ...
  return (
    <Card>
      <h1 className="text-xl font-bold mb-10">مدیریت کمپین‌ها</h1>
