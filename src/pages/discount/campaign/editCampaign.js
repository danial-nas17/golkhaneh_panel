...
const EditCampaign = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const discountRes = await api.get(`/panel/campaigns/${id}`);
        ...
        message.error("خطا در دریافت اطلاعات کمپین");
        console.error(error);
      } finally {
  ...
  const onFinish = async (values) => {
    try {
      ...
      await api.put(`/panel/campaigns/${id}`, payload);
      message.success(" کمپین با موفقیت ویرایش شد");
      navigate("/campaigns");
    } catch (error) {
      message.error("خطا در ویرایش  کمپین");
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Card>
      <h2 className="mb-10 text-xl">ویرایش کمپین</h2>
      ...
      <Form form={form} layout="vertical" onFinish={onFinish}>
        ...
        <Form.Item>
          ...
          <Button onClick={() => navigate("/campaigns")}>انصراف</Button>
        </Form.Item>
      </Form>
    </Card>
  );
