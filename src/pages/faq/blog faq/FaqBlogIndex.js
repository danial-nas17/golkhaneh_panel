...
const FaqBlogList = () => {
  ...
  const fetchFaqs = async (page = 1, pageSize = 7) => {
    setLoading(true);
    try {
      const response = await api.get(`/panel/faq?page=${page}&per_page=${pageSize}&faqable_type=Blog`);
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button 
            type="primary" 
            icon={<EditOutlined />}
            onClick={() => navigate(`/faq/blog/edit/${record.id}`)}
          >
            ویرایش
          </Button>
          <Button 
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 className='mb-10 text-xl'> سوالات متداول بلاگ</h2>
        <Button 
          type="primary" 
          icon={<PlusOutlined />}
          onClick={() => navigate('/faq/blog/add')}
        >
          افزودن سوال جدید
        </Button>
      </div>
