...
const UnreadNotificationsPage = () => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await api.get(
          "/panel/user/unread/notifications?per_page=all&page"
        );
        setNotifications(response.data.unread_user_notifications);
      } catch (error) {
  ...
  return (
    <Card>
      <h1 className="text-lg font-bold mb-4">Unread Notifications</h1>
      <Table
